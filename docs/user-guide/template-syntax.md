...
在参数化 SQL 中，支持使用以 `_` 开头的内置系统参数，运行时由服务端自动注入（无需在参数列表中定义，LLM 也无法干预）：

| 系统参数 | 说明 | 示例 |
| :--- | :--- | :--- |
| `{{_user.id}}` | 当前调用者的用户 ID | `usr_0194a2b3...` |
| `{{_user.name}}` | 当前调用者的用户名 / 账号 | `admin` |
| `{{_user.display_name}}` | 当前调用者的显示名称 | `张三` |
| `{{_now}}` | 当前日期时间（`yyyy-MM-dd HH:mm:ss`） | `2026-08-22 15:30:00` |
| `{{_date}}` | 当前日期（`yyyy-MM-dd`） | `2026-08-22` |
...
> [!TIP]
> - 系统参数同样支持默认值语法，如 `{{_user.id:anonymous}}`。当未登录或无用户上下文时，将自动降级为默认值。
> - 在 SQL 编辑器中键入 `{{_` 会自动触发系统参数代码补全。
