...
const form = reactive<{ tables: Record<string, any>[] }>({
  tables: [
    { data_source_id: "", schema: "", table: "", description: "", aliases: [] },
  ],
});

const canAddRow = computed(() => {
  const last = form.tables[form.tables.length - 1];
  return !!last && !!last.data_source_id && !!last.table;
});
...
defineExpose({
  getArgs: () => ({ tables: form.tables.map(stripEmpty) }),
});
</script>
    ...
    >
      ...
      <div class="flex gap-2">
        <el-select
          ...
          @change="entry.table = ''; entry.description = ''; entry.aliases = []"
        >
          <el-option
