...
<template>
  <div
    class="h-full max-w-6xl xl:max-w-7xl 2xl:max-w-[1440px] mx-auto px-6 sm:px-8 lg:px-12 py-6 flex flex-col justify-evenly gap-6"
  >
    <!-- 1. Hero Section -->
    <div
      class="flex flex-col items-center text-center space-y-3.5 max-w-2xl xl:max-w-3xl mx-auto"
    >
      <!-- Brand Pill Badge -->
      <div
        class="inline-flex items-center gap-2 px-3.5 py-1 rounded-full border border-[var(--ep-border-color-lighter)] bg-[var(--ep-bg-color)] text-xs lg:text-sm text-[var(--ep-text-color-secondary)]"
      >
        <img src="/dati.svg" alt="DatI" class="w-4 h-4 object-contain" />
        <span class="font-semibold text-[var(--ep-text-color-primary)]"
          >DatI</span
        >
        <span
          class="w-1.5 h-1.5 rounded-full bg-[var(--ep-color-primary)]"
        ></span>
      ...
      <!-- Main Headline -->
      <h1
        class="text-2xl sm:text-3xl lg:text-4xl font-bold tracking-tight text-[var(--ep-text-color-primary)] leading-tight"
      >
        {{ t("home.heroTitle") }}
      </h1>
      ...
      <!-- Subtitle -->
      <p
        class="text-xs sm:text-sm lg:text-base text-[var(--ep-text-color-regular)] leading-relaxed max-w-xl xl:max-w-2xl"
      >
        {{ t("home.heroDesc") }}
      </p>
    ...
    <div class="space-y-4">
      ...
      <div class="text-center">
        <h2
          class="text-base lg:text-lg font-bold text-[var(--ep-text-color-primary)] tracking-tight"
        >
        ...
        >
          <div class="space-y-2.5">
            ...
            <div class="flex items-center gap-2.5">
              <div
                class="w-8 h-8 lg:w-9 lg:h-9 shrink-0 rounded-lg flex items-center justify-center bg-[var(--ep-color-primary-light-9)] text-[var(--ep-color-primary)]"
              >
                <span
                  v-if="step.iconClass"
                  :class="[step.iconClass, 'text-base lg:text-lg']"
                ></span>
              ...
              <h3
                class="text-sm lg:text-base font-semibold text-[var(--ep-text-color-primary)] flex items-center gap-1 group-hover:text-[var(--ep-color-primary)] transition-colors"
              >
            ...
            <p
              class="text-xs lg:text-sm text-[var(--ep-text-color-regular)] leading-relaxed line-clamp-2 min-h-[36px] lg:min-h-[40px]"
            >
              {{ t(step.descKey) }}
            </p>
          </div>
          ...
          <div
            class="p-2.5 rounded-md bg-[var(--ep-fill-color-light)] flex items-center justify-between text-xs lg:text-sm"
          >
            <span class="text-[var(--ep-text-color-secondary)]">{{
              t(step.statLabelKey)
            }}</span>
            ...
            <span
              v-else
              class="font-medium text-[var(--ep-text-color-primary)] truncate max-w-[150px]"
            >
              {{ step.countText }}
            </span>
          </div>
    ...
    <div
      class="flex flex-wrap items-center justify-center gap-6 md:gap-10 text-xs lg:text-sm text-[var(--ep-text-color-secondary)] pt-3 border-t border-[var(--ep-border-color-lighter)]"
    >
      <el-tooltip :content="t('home.features.securityDesc')" placement="top">
        <div
          class="flex items-center gap-2 cursor-help hover:text-[var(--ep-text-color-primary)] transition-colors"
        >
      ...
      <el-tooltip
        :content="t('home.features.semanticPrecisionDesc')"
        placement="top"
      >
        <div
          class="flex items-center gap-2 cursor-help hover:text-[var(--ep-text-color-primary)] transition-colors"
        >
          <el-icon class="text-[var(--ep-color-primary)]"><Aim /></el-icon>
          <span>{{ t("home.features.semanticPrecision") }}</span>
        </div>
      </el-tooltip>
      <el-tooltip
        :content="t('home.features.flexibleIntegrationDesc')"
        placement="top"
      >
        <div
          class="flex items-center gap-2 cursor-help hover:text-[var(--ep-text-color-primary)] transition-colors"
        >
          <el-icon class="text-[var(--ep-color-primary)]"
            ><Connection
          /></el-icon>
          <span>{{ t("home.features.flexibleIntegration") }}</span>
        </div>
      </el-tooltip>
    </div>
  </div>
